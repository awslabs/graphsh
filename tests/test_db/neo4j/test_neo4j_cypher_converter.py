"""
Integration tests for Neo4j Cypher converter.

These tests verify that the converter correctly transforms Neo4j Cypher responses
...
def test_neo4j_cypher_converter(test_name):
    """Test that the converter correctly transforms Neo4j Cypher responses."""
    input_data, expected_output = load_test_data(test_name)

    # Create converter instance
    converter = Neo4jCypherConverter()

    # Convert the input data
    # Neo4j converter expects the results to be in the "results" key
    results = input_data.get("results", [])
    actual_output = converter.convert_results(results)
    actual_json = {"results": dict_graph_element(actual_output)}
